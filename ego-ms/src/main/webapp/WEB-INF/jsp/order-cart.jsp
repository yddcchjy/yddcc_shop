...
<form id="orderForm" class="hide" action="/order/create.html" method="post">
		...
		<%-- <c:forEach items="${msitemlist }" var="msitem" varStatus="status"> --%>
			<c:set var="totalPrice"  value="${ (msitem.msPrice * 1)}"/>
			<input type="hidden" name="orderItem.itemId" value="${msitem.id}"/>
			<!-- 数量为1 -->
			<input type="hidden" name="orderItem.num" value="1"/>
			<input type="hidden" name="orderItem.price" value="${msitem.msPrice}"/>
			<input type="hidden" name="orderItem.totalFee" value="${msitem.msPrice * 1}"/>
			<input type="hidden" name="orderItem.title" value="${msitem.title}"/>
			<input type="hidden" name="orderItem.picPath" value="${msitem.images[0]}"/>
			<input type="hidden" name="stockcount" value="${msitem.num}"/>
		<%-- </c:forEach> --%>
		<!-- ??? -->
		...
		<input type="hidden" name="orderShipping.receiverName" value="yddcc"/>
		<input type="hidden" name="orderShipping.receiverMobile" value="12312341234"/>
   			...
   			<ul class="ui-switchable-panel-main" id="consignee-list">
				...
				<li class="ui-switchable-panel" id="consignee_index_137617472"
					...
					<div class="consignee-item item-selected"
						...
						<div class="user-name">
							<div class="fl">
								<strong limit="4">yddcc</strong>&nbsp;&nbsp;收
							</div>
							<div class="fr">12312341234</div>
							<div class="clr"></div>
						</div>
						<div class="mt10" limit="15">广州 天河区 </div>
    ...
    <div class="goods-suit goods-last">
	 ...
	 <%-- <c:forEach items="${cartList }" var="cart"> --%>
		<div class="goods-item goods-item-extra">
			...
			<div class="p-img">
				<a target="_blank" href="/item/${msitem.id}.html">
					<img src="${msitem.images[0]}" alt="">
				</a>
			</div>
			<div class="goods-msg">
				<div class="p-name">
					<a href="/item/${msitem.id}.html" target="_blank">
						${msitem.title } 
					</a>
				</div>
				<div class="p-price">
					...
					<strong>￥<fmt:formatNumber
							...
							minFractionDigits="2" value="${msitem.msPrice / 100 }" /></strong>
					<!--增加预售金额显示 end-->
					<!-- 数量为1 -->
					<span class="ml20"> x${1} </span> 
					...
					<c:if test="${leftnum <= 0}">
						<span class="num" style="color:red">无货</span>
					</c:if>
